...
namespace DaproWindowsFormsApp
{
    public partial class ShowDataFromDatabaseForm : Form
    {
        ...
        public ShowDataFromDatabaseForm()
        {
            ...
            chosenDataFileGridView.Dock = DockStyle.Fill; // alebo iné nastavenie pozície
            chosenDataFileGridView.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            chosenDataFileGridView.MultiSelect = false;
        ...
        private void LoadDataFromTable(string tableName)
        {
            try
            {
                UpdateStatus($"Načítavam dáta z tabuľky {tableName}...");

                // Úplné vyčistenie DataGridView
                DataGridViewShowTableData.DataSource = null;
                DataGridViewShowTableData.Columns.Clear();
                DataGridViewShowTableData.Rows.Clear();
                DataGridViewShowTableData.Refresh();
                bindingSource.DataSource = null;

                // Načítanie dát do novej inštancie DataTable
                using (var connection = new SqliteConnection(ConnectionString))
                {
                    ...
                    // Nastavenie bindingSource a DataGridView
                    bindingSource.DataSource = dataTable;
                    DataGridViewShowTableData.DataSource = bindingSource;

                    UpdateStatus($"Počet záznamov: {dataTable.Rows.Count}");
                    NumberOfPatientsInTable(); // Call the method to update the number of patients
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Chyba pri načítaní dát: {ex.Message}", "Chyba",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                UpdateStatus("Chyba pri načítaní dát");
            }
        }
